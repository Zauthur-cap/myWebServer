...
void WebServer::trig_mode(){
    ...
    //LT+ET
    else if (1 == m_actormodel)
    {
        m_LISTENTrigmode = 0;
        m_CONNTrigmode = 1;
    }
    //ET+LT
    else if (2 == m_actormodel)
    {
        m_LISTENTrigmode = 1;
        m_CONNTrigmode = 0;
    }
    //ET+ET
    else if (3 == m_actormodel)
    {
        m_LISTENTrigmode = 1;
        m_CONNTrigmode = 1;
    }
...
void WebServer::thread_pool(){
    m_pool = new threadpool<http_conn>(m_actormodel, m_connPool);
}
...
void WebServer::eventListen(){
    ...
    //优雅关闭连接, 优雅关闭连接是指等待所有数据发送完毕或者等待一段时间后强制关闭连接
    if(m_OPT_LINGER == 0){
    ...
    assert(ret >= 0); //断言,如果ret小于0,则终止程序
    //监听
